...
#  for i in range(1000, 1001):
    ...
    for j in range(1, len(results)):
      ...
      if buy > 0:
        ...
        buy = 0.0
        sell = 0.0
      if isBan(yes_price, current_price) and not isBan(yes_price, open_price):
        history_list.append(True)
      else:
        history_list.append(False)
      if len(history_list) > 2:
        if history_list[0] and not history_list[1]:
          if isBan(yes_price, current_price):
            print day + " " + stock_id + " yes"
          elif isBan(yes_price, highest_price):
            print day + " " + stock_id + " no"
          if not day_count_dict.has_key(day) and isBan(yes_price, highest_price):
            buy =  highest_price
