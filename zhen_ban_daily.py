...
#  for i in range(0, 1):
    ...
    try:
      db_cur.execute('select date,current_price,highest_price,lowest_price,yesterday_close_price from daily where stock_id = %s and date > %s', [stock_id, beforestr])
    except MySQLdb.Error,e:
      sys.stderr.write("Mysql Error %d: %s\n" % (e.args[0], e.args[1]))
    results = db_cur.fetchall()
    
    for j in range(0, len(results)):
      yes_price = float(results[j][4])
