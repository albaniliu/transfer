...
#  for i in range(1000, 1001):
    ...
    for j in range(1, len(results)):
      ...
      if open_price > 0 and buy > 0:
        ...
        #  sell = open_price
        #  print "open"
        if highest_price > open_price * 1.08:
          sell = open_price * 1.08
          #print "high"
        else:
          sell = current_price
          #print "close"
        sell = open_price
#        if isBan(yes_price, current_price) and j +1 < len(results):
#          sell = results[j + 1][3]
      ...
      if isBan(yes_price, lowest_price):
        ...
        #if lian_ban_count > 1 and isBan(yes_price, open_price):
        if lian_ban_count > 2 and not isBan(yes_price, open_price):
        #if lian_ban_count > 2 and not isBan(yes_price, open_price) and open_price / yes_price < 1.05:
        #if lian_ban_count > 2:
         # print day + " " + stock_id +  " " + str(open_price / yes_price) + " " + str(isBan(yes_price, current_price))
          ...
          #if isBan(yes_price, highest_price) and not day_count_dict.has_key(day):
          if not day_count_dict.has_key(day):
            if open_price * 0.95 > lowest_price:
          #print str(open_price / yes_price) + " " + str(lowest_price / yes_price)
              buy = open_price * 0.95
              #print day + " " + stock_id + " buy"
            #else:
            #  buy = current_price
            #print day + " " + stock_id
            #buy = current_price
            day_count_dict[day] = 1
        max_lian_ban = max(max_lian_ban, lian_ban_count)
        lian_ban_count = 0
