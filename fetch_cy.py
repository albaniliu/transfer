    ...
    if (i+1) % 900 == 0 or i == len(stock_ids) - 1:
      ...
      for content in contents:
        ...
        try:
          ...
          if closed_price == 0: continue

          cur_total_quantity = long(tmp[2])
          last_total_quantity = long(value_list[2])
          time = allCol[31]

          if (time > '09:45:00' and cur_total_quantity > 5000000 and (cur_total_quantity - last_total_quantity) * 1.0 / cur_total_quantity > 0.1):
            print stock_id + " " + allCol[0] + " " + str((cur_total_quantity - last_total_quantity)/100) + " " + str(cur_total_quantity/100) + " f"
            sys.stdout.flush()
    ...
    if (i+1) % 900 == 0 or i == len(stock_ids) - 1:
      ...
      for content in contents:
        ...
        value = [stock_id,allCol[0], allCol[1], allCol[2], allCol[3],
                 ...
                 allCol[29], allCol[30], allCol[31], allCol[32]]
        try:
          results = db_cur.execute("insert into chuangye(stock_id,name,open_price,yesterday_close_price,current_price, \
